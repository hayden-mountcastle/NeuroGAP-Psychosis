...
## Analysis with clean/unclean cases and controls

Distribution of ltl/age with clean cases , unclean cases, controls 
Update powerpoint with controls as reference group
...
The unclean cases among Females screw older than the rest of the population 
```{r, warning=FALSE}
...
tbl <- table1(~ age_at_iview + educ_ord + 
               ...
               cidi_q13 + cidi_q14 + cidi_q15 + 
                cidi_q16 + cidi_q17 | msex*is_case_clean,
...
mean(tel_all_nomissing_2$age_at_iview)
sd(tel_all_nomissing_2$age_at_iview)
min(tel_all_nomissing_2$age_at_iview)
max(tel_all_nomissing_2$age_at_iview)
...
kable(df, format = "html", caption="Sets ACE") %>%
  kable_styling(full_width = FALSE)
...
# tbl_regression(model4_all) %>% add_significance_stars(
#     hide_p = FALSE, hide_ci = FALSE,
#     pattern = "{p.value}{stars}"
#   )
...
#model2_male <-  lm(ltl_adj2 ~ age_at_iview + is_case_clean + msex, ros_merged_male)
#summary(model)
...
table1(~  factor(msex) + age_at_iview | is_case_f, data=tel_all_nomissing)
